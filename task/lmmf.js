...
exports.dist = (cb) => {
    let htmlPath = null;
    if (conf.config.server.startPath && conf.config.server.startPath !== '/' && conf.config.server.startPath !== '.') {
        htmlPath = path.resolve(conf.cwd, conf.config.src.html, conf.config.server.startPath);
    }
